  ...
  return (
    <Box maxWidth="md" sx={{ p: 2 }} component="form" onSubmit={formik.handleSubmit}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', mb: 2 }}>
        <Typography onClick={() => navigate('/Cars')} sx={{ cursor: 'pointer', color: theme.palette.primary.main }}>
          {t('Cars')}
        </Typography>
      ...
      {/* Car License */}
      <Typography variant="h5" color="primary" gutterBottom>{t('Car License')}</Typography>
      <Box sx={{ height: 2, backgroundColor: theme.palette.secondary.sec, mb: 2, borderRadius: 1 }} />
      <Typography gutterBottom>{t('Car License Picture')}</Typography>
      <Card sx={{ backgroundColor: theme.palette.secondary.sec, borderRadius: 1, mb: 3 }}>
        <CardContent sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography>{t('maxSizeText')}</Typography>
          <Button variant="contained" startIcon={<AddIcon sx={{ [isArabic ? 'ml' : 'mr']: 1 }} />}>
            {t('Upload Files')}
          </Button>
        </CardContent>
      </Card>
      ...
      {/* Expired Date */}
      <Box sx={{ mb: 3 }}>
        <Typography>{t('Expired Date')}<Typography component="span" sx={{ color: 'error.main' }}>*</Typography></Typography>
        <TextField
          fullWidth
          ...
          error={formik.touched.licenseExpiredDate && Boolean(formik.errors.licenseExpiredDate)}
          helperText={formik.touched.licenseExpiredDate && formik.errors.licenseExpiredDate}
        />
      </Box>

      {/* Car Details */}
      <Typography variant="h5" color="primary" gutterBottom>{t('Car Details')}</Typography>
      <Box sx={{ height: 2, backgroundColor: theme.palette.secondary.sec, mb: 2, borderRadius: 1 }} />
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {/* Model */}
        <Grid item xs={12} md={6}>
          <Typography>{t('Car Model')}<Typography component="span" sx={{ color: 'error.main' }}>*</Typography></Typography>
          <TextField
            fullWidth
            name="carModel"
        ...
        {/* Company/Personal */}
        <Grid item xs={12} md={6}>
        <Typography>{t('Company Car')}<Typography component="span" sx={{ color: 'error.main' }}>*</Typography></Typography>

          <FormControl fullWidth variant="filled" error={formik.touched.ownershipType && Boolean(formik.errors.ownershipType)}>
            <Select
              name="ownershipType"
              displayEmpty
        ...
        {/* Plate */}
        <Grid item xs={12} md={6}>
          <Typography>{t('Plate Number')}<Typography component="span" sx={{ color: 'error.main' }}>*</Typography></Typography>
          <TextField
            fullWidth
            name="plateNumber"
        ...
        {/* Color */}
        <Grid item xs={12} md={6}>
          <Typography>{t('Car Color')}<Typography component="span" sx={{ color: 'error.main' }}>*</Typography></Typography>
          <TextField
            fullWidth
            name="carColor"
        ...
        {/* Type */}
        <Grid item xs={12} md={6}>
        <Typography>{t('Car Type')}<Typography component="span" sx={{ color: 'error.main' }}>*</Typography></Typography>

          <FormControl fullWidth variant="filled" error={formik.touched.carType && Boolean(formik.errors.carType)}>
            <Select
              name="carType"
              displayEmpty
        ...
        {/* Year */}
        <Grid item xs={12} md={6}>
          <Typography>{t('Car Year')}<Typography component="span" sx={{ color: 'error.main' }}>*</Typography></Typography>
          <TextField
            fullWidth
            name="carYear"
      ...
      {/* Car Pictures */}
      <Typography variant="h5" color="primary" gutterBottom>{t('Car Picture')}</Typography>
      <Box sx={{ height: 2, backgroundColor: theme.palette.secondary.sec, mb: 2, borderRadius: 1 }} />
          ...
          <Grid item xs={12} sm={6} key={field}>
            <Card sx={{ p: 2, borderRadius: 1 }}>
              <Box display="flex" alignItems="center" mb={1}>
                ...
                <Typography sx={{ [isArabic ? 'mr' : 'ml']: 1 }}>{t(`Car’s Picture ${field}`)}</Typography>
              </Box>
              {images[field] ? (
                <Box
                sx={{
              ...
              >
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                  <Typography>{t('maxSizeText')}</Typography>
                  <Button component="label" variant="contained" startIcon={<AddIcon sx={{ [isArabic ? 'ml' : 'mr']: 1 }} />}>
                    {t('Upload Files')}
